...
- The GUI never writes directly to `/etc/swanctl`.
...
- Profile deletion accepts UUIDs only and deletes only UUID-named files under
  `/etc/swanctl/conf.d/gic-ipsec` and `/etc/swanctl/secrets.d/gic-ipsec`.
...
Root-side strongSwan secret files exist because strongSwan needs them. They are
written by the helper with mode `0600` and root ownership when the helper runs as
root.
